...
net_properties = NetProperties(64, 64, 32, 400, 400, 128)
network = Network(net_properties)
network.load('src/network3.model')
...
class DepModel:
    ...
    def score(self, str_features):
        ...
        features = []
        for i in range(0, 20):
            if str_features[i] in word_dict.keys():
                features.append(word_dict[str_features[i]])
            else:
                features.append(word_dict['<unk>'])

        for i in range(20, 40):
            if str_features[i] in pos_dict.keys():
                features.append(pos_dict[str_features[i]])
            else:
                features.append(pos_dict['<null>'])
        
        for i in range(40, 52):
            features.append(labels_dict[str_features[i]])

        scores = network.decode(features)
        # print("*************************************",scores)
        # pdb.set_trace()
